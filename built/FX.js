...
export class FX {
    constructor(node) {
        ...
        if (this.fxtype === undefined)
            throw new TypeError("Node given is not a supported type !");
        if (this.fxtype === "constant") {
            this.consttype = CONST_NODE_TYPE[0];
            this.constdata = null;
        }
        if (this.fxtype in FX_DRAW)
            this.draw = () => FX_DRAW[this.fxtype](this.name);
    ...
    connectParam(fx, param, output = 0) {
        if (MODULATIONS[fx.fxtype].indexOf(param) === -1)
            throw new TypeError(param + " is not a param of the fx type " + fx.fxtype + " !");
    ...
    isParamConnected(param) {
        for (const k in this.inputParams) {
            if (this.inputParams[k].param == param)
                return true;
        }
        return false;
    }
    disconnectAll(params = true) {
        ...
        if (params)
            for (let k in this.outputParams) {
                const v = this.outputParams[k];
                delete v.fx.inputParams[this.name + v["param"] + v["idx"]];
            }
        this.outputs = [];
...
export function copyFxs(...fxs) {
    ...
    if (fxs.length == 0)
        return res;
    if (fxs.length == 1 && fxs[0].constructor === Array)
        fxs = fxs[0];
    for (const fx of fxs) {
        res[fx.name] = fx.copy();
    }
...
export class FXGraph {
    constructor(context, drawflow, defaultFxs) {
        ...
        for (let n of defaultFxs) {
            if (this.defaultNodes[n.label] !== undefined)
                throw new Error("Default fxs must have different labels !");
        ...
        this.drawflow.on("connectionCreated", (e) => {
            const out = this.getAudioNodeFromId(e.output_id), input = this.getAudioNodeFromId(e.input_id);
            out.connect(input);
        });
            ...
            for (let k in this.defaultNodes) {
                if (this.defaultNodes[k].gid === e) {
                    this.defaultNodes[k].gid =
                        this.drawflow.addNode(this.defaultNodes[k].name, this.defaultNodes[k].node.numberOfInputs, this.defaultNodes[k].node.numberOfOutputs, 300, 200, "", { node: this.defaultNodes[k].name }, "<span id='graph_" + this.defaultNodes[k].name + "'>" + this.defaultNodes[k].label, false);
        ...
        let y = 0, x = 0;
        for (let k in this.defaultNodes) {
            this.defaultNodes[k].gid = this.drawflow.addNode(this.defaultNodes[k].name, this.defaultNodes[k].node.numberOfInputs, this.defaultNodes[k].node.numberOfOutputs, x * 300, y * 100, "", { node: this.defaultNodes[k].name }, "<span id='graph_" + this.defaultNodes[k].name + "'>" + this.defaultNodes[k].label + "</span>", false);
    ...
    addNode(node) {
        ...
        fx.gid = this.drawflow.addNode(fx.name, node.numberOfInputs, node.numberOfOutputs, 0, 200, "", { node: fx.name }, "<span id='graph_" + fx.name + "'>" + fx.label + "</span>", false);
        return fx.gid;
    }
    deleteNode(name) {

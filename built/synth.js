...
let masterFader = new GainNode(AC), masterLimiter = new WaveShaperNode(AC), masterOscilloscope = new AnalyserNode(AC), masterFFTAnalyser = new AnalyserNode(AC);
...
function initAudio() {
    ...
    fx = new FXGraph(AC, drawflow, [osc1, adsr,
...
function playOsc(note = 69) {
    if (AC === null)
        return;
    if (osc[note] !== undefined)
        return;
    const nodes = copyFxs(fx.getAllNodes().concat(getModulationNodes()));
    const attack = Number(getI("attack").value) / 1000, decay = Number(getI("decay").value) / 1000, sustain = Number(getI("sustain").value);
...
function stopOsc(note = 69) {
    ...
    for (let k in osc[note]) {
        if (osc[note][k].fxtype == 'oscillator')
            nodes[k].node.stop(AC.currentTime + release);
    }
    delete osc[note];
}
function updateADSR() {
...
function mainloop() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const h = canvas.height * .95, hh = Math.floor(canvas.height / 2 * .95), w = canvas.width, ww = Math.floor(canvas.width / 2);
        ctx.fillStyle = "white";
        ...
        ctx.stroke();
        {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                let x1 = ww + linearToLog(i * width + 1, 1, ww), x2 = ww + linearToLog((i + 1) * width + 1, 1, ww);
                y -= 2 * h;
...
window.onload = () => {
    ...
    const KEYS = ['<', 'w', 's', 'x', 'd', 'c', 'v', 'g', 'b', 'h', 'n', 'j', ',', ';', 'l', ':', 'm', '!'], KEY_START = 11;
